...
exports.register = function(req, res, next) {
	console.log("here");
	console.log("Details",req.body.email, req.body.password, req.body.name)
				...
				if(user){
                    ...
                    async.waterfall([
                        ...
                        function (user, subscription, callback) {
                            ...
                            console.log('chargebee_customer_id', subscription.customer.id);
                            console.log('chargebee_subscription_id', subscription.subscription.id);
                            console.log('chargebee_invoice_id', subscription.invoice.id);
                            console.log('user_id', user);

                            userSubscription = new UserSubscription({
                                user_id: user._id, chargebee_customer_id: subscription.customer.id,
                                chargebee_subscription_id: subscription.subscription.id, chargebee_invoice_id: subscription.invoice.id
                            });
                        ...
                        if(err) {
                            return res.status(400).jsonp({status: false, message: "There is an interna server error", err: err});  
                        }
