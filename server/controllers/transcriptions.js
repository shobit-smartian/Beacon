...
exports.uploadAudio = function(req,res,next){
    ...
    const interview = new Interview({markers: req.body.timeStamps, blob_str: uploadPath, user:req.params.user, media_length:req.body.length});
...
exports.fetchAudio = function (req, res) { Interview.findById(req.params.interview, function (err, result) { if(err) { return res.status(400).json({success: false, message: "Error"}); } return res.status(200).json({success: true, data: result}); }) }
